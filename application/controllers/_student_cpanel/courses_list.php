...
$query_RsCoursesList = sprintf("SELECT `eschool_courses`.*,`eschool_subjects`.* FROM `eschool_courses`,`eschool_subjects` WHERE `eschool_courses`.`SubjectID` = `eschool_subjects`.`SubjectID` AND `eschool_courses`.EndDate >=CURDATE() ORDER BY `eschool_courses`.CourseID DESC");

/* $query_RsCoursesList = sprintf("SELECT `eschool_courses`.*,`eschool_subjects`.* FROM `eschool_courses`,`eschool_subjects` WHERE `eschool_courses`.`SubjectID` = `eschool_subjects`.`SubjectID`  AND `eschool_courses`.EndDate >=NOW() ORDER BY `eschool_courses`.CourseID DESC");
 */
...
mysql_select_db($database_conn, $conn);
$query_RsCoursesList_Expired = sprintf("SELECT `eschool_courses`.*,`eschool_subjects`.* FROM `eschool_courses`,`eschool_subjects` WHERE `eschool_courses`.`SubjectID` = `eschool_subjects`.`SubjectID`  AND `eschool_courses`.EndDate <CURDATE() ORDER BY `eschool_courses`.CourseID DESC");
$RsCoursesList_Expired  = mysql_query($query_RsCoursesList_Expired, $conn) or die(mysql_error());
$row_RsCoursesList_Expired  = mysql_fetch_assoc($RsCoursesList_Expired );
