...
$query_RsCoursesList = sprintf("SELECT `eschool_courses`.*,`eschool_subjects`.*,user_accounts.Username FROM `eschool_courses`,`eschool_subjects`,user_accounts  WHERE `eschool_courses`.`SubjectID` = `eschool_subjects`.`SubjectID` AND `eschool_courses`.TeacherUserID=user_accounts.UserID  ORDER BY `eschool_courses`.CourseID DESC", GetSQLValueString($colname_RsCoursesList, "int"));
