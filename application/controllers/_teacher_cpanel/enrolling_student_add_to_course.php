...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "frmAddThisStudentToCourse")) {
	...
	CreateAt) VALUES (%s,%s,%s,%s,%s)",
	  ...
	  $insertGoTo = "index.php?page_id=_teacher_cpanel/enrolled_student_list&STATUS=STUDENTSADDED&CourseID=".$_POST['CourseID'];
	/*   if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_conn, $conn);
$query_RsGetStudentDetails = sprintf("SELECT DISTINCT user_accounts.* FROM 
enrolled_students_list,user_accounts
WHERE
user_accounts.UserID=enrolled_students_list.UserID
AND enrolled_students_list.CourseID<>%s 
ORDER BY user_accounts.UserID DESC",GetSQLValueString($_POST['CourseID'], "int"));
