...
mysql_select_db($database_conn, $conn);
if(isset($_SESSION['MM_UserID']) && $_SESSION['MM_UserGroup']==1) { // For Administrator Full Access to all courses
$query_RsGetTeachersCourses = sprintf("SELECT * FROM eschool_courses WHERE EndDate>=CURDATE()  ORDER BY CourseID DESC", GetSQLValueString($colname_RsGetTeachersCourses, "int"));
}else{// this teacher only
$query_RsGetTeachersCourses = sprintf("SELECT * FROM eschool_courses WHERE EndDate>=CURDATE() AND TeacherUserID = %s ORDER BY CourseID DESC", GetSQLValueString($colname_RsGetTeachersCourses, "int"));	
