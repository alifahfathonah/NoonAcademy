...
if($SelectedCourseID==0 && isset($_SESSION['MM_UserGroup']) && $_SESSION['MM_UserGroup']==2) { // for Teacher
	$query_RsGetMyScheduledClasses = sprintf("SELECT * FROM eschool_courses_classes WHERE TeacherUserID = %s AND eschool_courses_classes.WizIQ_start_time>=NOW()", GetSQLValueString($colname2_RsGetMyScheduledClasses, "int"));
}else if($SelectedCourseID>0 && isset($_SESSION['MM_UserGroup']) && $_SESSION['MM_UserGroup']==2) { // For Teacher With Course ID
		$query_RsGetMyScheduledClasses = sprintf("SELECT * FROM eschool_courses_classes WHERE TeacherUserID = %s AND CourseID=%s  AND eschool_courses_classes.WizIQ_start_time>=NOW()", GetSQLValueString($colname2_RsGetMyScheduledClasses, "int"),GetSQLValueString($SelectedCourseID, "int"));
}else  if($SelectedCourseID==0 && isset($_SESSION['MM_UserGroup']) && $_SESSION['MM_UserGroup']==1) { // For Administrator
	
	$query_RsGetMyScheduledClasses = sprintf("SELECT * FROM eschool_courses_classes WHERE   eschool_courses_classes.WizIQ_start_time>=NOW() ");
}else  if($SelectedCourseID>0 && isset($_SESSION['MM_UserGroup']) && $_SESSION['MM_UserGroup']==1) { // For Administrator with Course ID

	$query_RsGetMyScheduledClasses = sprintf("SELECT * FROM eschool_courses_classes WHERE CourseID=%s AND eschool_courses_classes.WizIQ_start_time>=NOW()",GetSQLValueString($SelectedCourseID,"int"));
}
...
$RsGetMyScheduledClasses_Expired = mysql_query($query_RsGetMyScheduledClasses, $conn) or die(mysql_error());
$row_RsGetMyScheduledClasses_Expired  = mysql_fetch_assoc($RsGetMyScheduledClasses_Expired );
$totalRows_RsGetMyScheduledClasses_Expired  = mysql_num_rows($RsGetMyScheduledClasses_Expired );
