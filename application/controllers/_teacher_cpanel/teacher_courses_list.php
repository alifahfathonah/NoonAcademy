...
$query_RsCoursesList = sprintf("SELECT `eschool_courses`.*,`eschool_subjects`.* FROM `eschool_courses`,`eschool_subjects` WHERE `eschool_courses`.`SubjectID` = `eschool_subjects`.`SubjectID` AND TeacherUserID = %s AND `eschool_courses`.EndDate >=CURDATE() ORDER BY `eschool_courses`.CourseID DESC", GetSQLValueString($colname_RsCoursesList, "int"));
...
$colname_RsCoursesListExpired = "-1";
if (isset($_SESSION['MM_UserID'])) {
  $colname_RsCoursesListExpired = $_SESSION['MM_UserID'];
}
mysql_select_db($database_conn, $conn);
$query_RsCoursesListExpired = sprintf("SELECT `eschool_courses`.*,`eschool_subjects`.* FROM `eschool_courses`,`eschool_subjects` WHERE `eschool_courses`.`SubjectID` = `eschool_subjects`.`SubjectID` AND TeacherUserID = %s AND `eschool_courses`.EndDate <CURDATE() ORDER BY `eschool_courses`.CourseID DESC", GetSQLValueString($colname_RsCoursesListExpired, "int"));

$RsCoursesListExpired = mysql_query($query_RsCoursesListExpired , $conn) or die(mysql_error());
$row_RsCoursesListExpired = mysql_fetch_assoc($RsCoursesListExpired);
