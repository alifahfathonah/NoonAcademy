	...
	private function GetSQLValueString($theValue, $theType, $theDefinedValue = "", $theNotDefinedValue = "") 
{
  ...
  return $theValue;
	...
	function AddAttendee($WizIQ_class_id){
		...
		//mysql_select_db($database_conn, $conn);
		$query_RsGetCourseEntrolledStudents = sprintf("
		SELECT 
    eschool_view_enrolled_students . *
FROM
    eschool_view_enrolled_students

LEFT JOIN
    eschool_courses_studdent_wiziq_classes
ON
    eschool_courses_studdent_wiziq_classes.StudentUserID = eschool_view_enrolled_students.StudentUserID
WHERE eschool_courses_studdent_wiziq_classes.StudentUserID IS NULL
		 AND
		eschool_view_enrolled_students.CourseID = %s", 
		...
		$language_culture_name="en-US";
		 do {  
			 ...
			 $XMLAttendee=$XMLAttendee .$row_RsGetCourseEntrolledStudents['FirstName']; 
		...
		"<textarea cols=\"80\" rows=\"8\">$XMLAttendee </textarea>";
