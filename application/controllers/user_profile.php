...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  ...
  $updateSQL = sprintf("UPDATE user_accounts SET FirstName=%s, MiddleNames=%s, LastNames=%s, AddressLine1=%s, AddressLine2=%s, City=%s, CountryID=%s, Phone=%s, Email=%s, WebSite=%s, CompanyNameIfAny=%s, ContactNo=%s, Others=%s  WHERE UserID=%s",
  ...
  $Result1 = mysql_query($updateSQL, $conn) or die(mysql_error());
    ...
    <script type="text/javascript">
 		$(document).ready(function(e) {
   		ZORKIF_ShowNotificationMessage("Profile updated successfully");
 	});
	</script>
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form2")) {
	...
	if (($UploadedFileName=fnUploadFileToPath('Picture',USER_PROFILE_UPLOAD_PATH))=='NOTALLOWED'){
	  ...
	  $updateSQL = sprintf("UPDATE user_accounts SET Picture=%s WHERE UserID=%s",
                       GetSQLValueString($UploadedFileName, "text"),
                       GetSQLValueString($_POST['UserID'], "int"));

  mysql_select_db($database_conn, $conn);
  $Result1 = mysql_query($updateSQL, $conn) or die(mysql_error());
	}// end of Saveing Record to Db
}
